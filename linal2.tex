...
-(\lambda - 1)^{\frac{n + 1}{2}}(\lambda + 1)^{\frac{n - 1}{2}}$ \\ \\ \\
...
\Rightarrow \lambda$ - собственное значение $A$. \\ \\ \\
...
(воспользовались тем, что $(PBP^{-1})^n = \underbrace{PBP^{-1}PB \cdots PBP^{-1}}_{\text{n раз}} = PB^nP^{-1}$)\\ \\ \\
...
Значит $g(\lambda)$ $\vdots$ $f(\lambda)$. \\ \\ \\ 
